...
# -*- coding: utf-8 -*-
"""
Strategy 17 — Method 1 & Method 2 — Variation 1
-------------------------------------------------
...
BASE_LOG_DIR = Path("./output/Strategy 17 M1 M2 Variation 1 Forex Live Logs")
...
def load_fired_events():
    ...
    for inst in INSTRUMENTS:
        path = BASE_LOG_DIR / f"Strategy17_M1M2_Var1_{inst}_SELL_5min.csv"
        if path.exists():
            try:
...
def format_telegram(r: dict, symbol: str) -> str:
    ...
    return (
        f"🔴 SELL {symbol}\n"
        f"Strategy: Strategy 17 M1 M2 Variation 1 LIVE\n"
...
def run_live_scan_for_instrument(symbol: str):
    ...
    csv_path = BASE_LOG_DIR / f"Strategy17_M1M2_Var1_{symbol}_SELL_5min.csv"
    ...
    for row in rows:
        ...
        if row.get("Status") != "Intrade" and "Invalidated" in row.get("Status", ""):
            ...
            process_telegram_exits(row, symbol, "sell", "Strategy 17 M1 M2 Variation 1", recent_1m)
...
def main():
    log("=" * 60)
    log("🚀 Strategy 17 M1 M2 Variation 1 FOREX LIVE SCANNER")
    log("=" * 60)
    
    load_fired_events()
    syms_str = ", ".join(INSTRUMENTS)
    tg_post(f"🚀 Strategy 17 M1 M2 Variation 1 LIVE — STARTED\nSymbols: {syms_str}\nMode: SELL side only")
