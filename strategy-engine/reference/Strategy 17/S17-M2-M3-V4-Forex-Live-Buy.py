...
# -*- coding: utf-8 -*-
"""
Strategy 17 — Method 3 & Method 2 — Variation 4
-------------------------------------------------
...
INSTRUMENTS = ["XAU_USD", "USD_JPY"]

BASE_LOG_DIR = Path("./output/Strategy 17 M3 M2 Variation 4 Forex Live Logs")
...
def load_fired_events():
    ...
    for inst in INSTRUMENTS:
        path = BASE_LOG_DIR / f"Strategy17_M3M2_Var4_{inst}_BUY_5min.csv"
        if path.exists():
            try:
...
def format_telegram(r: dict, symbol: str) -> str:
    ...
    return (
        f"🟢 BUY {symbol}\n"
        f"Strategy: Strategy 17 M3 M2 Variation 4 LIVE\n"
...
def run_live_scan_for_instrument(symbol: str):
    ...
    csv_path = BASE_LOG_DIR / f"Strategy17_M3M2_Var4_{symbol}_BUY_5min.csv"
    ...
    for edt, idxs in entry_map.items():
        ...
        for old_i in idxs_sorted[:-1]:
            old = rows[old_i]; new = rows[newest]
            old["Status"] = f"Invalidated due to Buy occured at same Datetime"
            extra = OrderedDict({
                "Invalidation Reason": old["Status"],
                f"{tc} MACD CYCLE start time due to which setup invalidated": new.get(f"{tc} MACD cycle Startime"),
                f"{tc} MACD CYCLE endtime time due to which setup invalidated": new.get(f"{tc} MACD cycle Endtime"),
                "Key candle datetime due to which setup invalidated": new.get("Final Key Candle Datetime"),
                f"Final buy datetime": edt,
            })
            old_add = old.get("Strategy Additional info") or ""
            old["Strategy Additional info"] = old_add + "\n\n" + vkv(extra)
            for prefix in ["2hrs", "4hrs", "1D"]:
            ...
            for r_ in RATIOS_FULL:
                ...
                if r_ >= 2: old[f"Status 1:{r_}"] = None
            for _, col in VOL_COL_MAP.items(): old[col] = None
            old["Final Buy found from which Method"] = None

    for row in rows:
        if row.get("Status") == "Intrade":
            apply_variation_4_logic(row, arr1, arr5, arr2h, arr4h, arr1d, volcalc1, side)

    # After V4 logic, we determine if it's a new final entry
    for row in rows:
        fcc_ts = row.get("Final Strategy Candle Datetime")
        row["Logged At UTC"] = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
        
        # Give it a unique ID based on FCC TS and symbol
        ev_id = hashlib.sha256(f"buy|{symbol}|{fcc_ts}".encode()).hexdigest()[:24]
        row["Event ID"] = ev_id
        ...
        if row.get("Status") != "Intrade" and "Invalidated" in row.get("Status", ""):
            ...
            process_telegram_exits(row, symbol, "buy", "Strategy 17 M2 M3 Variation 4", recent_1m)
...
def main():
    log("=" * 60)
    log("🚀 Strategy 17 M3 M2 Variation 4 FOREX LIVE SCANNER")
    log("=" * 60)
    
    load_fired_events()
    syms_str = ", ".join(INSTRUMENTS)
    tg_post(f"🚀 Strategy 17 M3 M2 Variation 4 LIVE — STARTED\nSymbols: {syms_str}\nMode: BUY side only")
