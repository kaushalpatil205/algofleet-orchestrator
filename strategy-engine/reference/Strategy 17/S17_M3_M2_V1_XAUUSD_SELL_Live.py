...
# -*- coding: utf-8 -*-
"""
Created on Mon May 25 08:25:45 2026
...
# -*- coding: utf-8 -*-
"""
Created on Tue May 19 13:00:45 2026
...
OANDA_API_KEY = "40f45812a032b3aceff401e5fe883dc3-96dd4738b7026f3530356618c5f1bdaf"
OANDA_ENV = "practice"
...
BASE_LOG_DIR = Path("./output/Strategy 17 M3 M2 Variation 1 Forex Live Logs")
BASE_LOG_DIR.mkdir(parents=True, exist_ok=True)

# Lookback candle counts
LOOKBACK_5M  = 2500
LOOKBACK_1M  = 6000
LOOKBACK_2H  = 500
LOOKBACK_4H  = 300
...
# ================================================================
# LIVE DATA FETCHING (OANDA)
# ================================================================

GRANULARITY_TO_DELTA = {
...
def to_oanda_time(dt_obj: datetime) -> str:
    return dt_obj.astimezone(timezone.utc).isoformat().replace("+00:00", "Z")

def fetch_oanda_chunk(instrument: str, granularity: str, start_dt: datetime, end_dt: datetime) -> pd.DataFrame:
    url = f"{OANDA_BASE_URL}/v3/instruments/{instrument}/candles"
    params = {
        "granularity": granularity,
        "price": "M",
        "from": to_oanda_time(start_dt),
        "to": to_oanda_time(end_dt),
    }
...
def fetch_live_oanda_candles(instrument: str, granularity: str, n_candles: int) -> pd.DataFrame:
    delta = GRANULARITY_TO_DELTA[granularity]
    now_utc = datetime.now(timezone.utc)
    # Add buffer to ensure we get exactly n_candles
    start_dt = now_utc - (delta * (n_candles + 10))
    
    all_parts = []
    current = start_dt
    while current <= now_utc:
        chunk_end = min(current + delta * 4999, now_utc)
        try:
            df_chunk = fetch_oanda_chunk(instrument, granularity, current, chunk_end)
            if not df_chunk.empty:
                all_parts.append(df_chunk)
        except Exception as e:
            log(f"Oanda fetch error [{instrument} {granularity}]: {e}")
            _time.sleep(2)
            continue
    ...
    df_all = df_all.tail(n_candles)
    return df_all.set_index("datetime")
...
# ================================================================
# INDICATORS — 5min / 1min  (XAUUSD-correct, ported from Strategy 14)
# ================================================================

# ── EMAKAMA parameters (mirrors Strategy 14) ─────────────────────
EK_ER_LEN   = 10
EK_FAST_LEN = 2
EK_SLOW_LEN = 30

# ── kama_lines parameters (mirrors Strategy 14) ──────────────────
KL_LENGTH      = 14
KL_FAST_LENGTH = 2
KL_SLOW_LENGTH = 30
KL_HP_PERIOD   = 48

# ── SMA parameters (mirrors Strategy 14) ─────────────────────────
SMA_KAMA_LENGTH = 5
SMA_KAMA_FAST   = 2.5
SMA_KAMA_SLOW   = 20
SMA_EMA_LENGTH  = 20
...
def calc_sma_kama(df: pd.DataFrame) -> pd.Series:
    """
    ...
    return _pine_ema_series(_pine_kama_series(hlc3), SMA_EMA_LENGTH)
...
def load_fired_events():
    ...
    for inst in INSTRUMENTS:
        path = BASE_LOG_DIR / f"Strategy17_M3M2_Var1_{inst}_SELL_5min.csv"
        if path.exists():
            try:
...
def format_telegram(r: dict, symbol: str) -> str:
    ...
    return (
        ...
        f"Qty: {r.get('Qty', 'N/A')}\n"
        f"Hard SL Price: {r.get('Hard SL Price', 'N/A')}\n"
...
def run_live_scan_for_instrument(symbol: str):
    global _fired_events
    log(f"Fetching live candles for {symbol}...")
    df5_raw = fetch_live_oanda_candles(symbol, "M5", LOOKBACK_5M)
    df1_raw = fetch_live_oanda_candles(symbol, "M1", LOOKBACK_1M)
    df2h_raw = fetch_live_oanda_candles(symbol, "H2", LOOKBACK_2H)
    df4h_raw = fetch_live_oanda_candles(symbol, "H4", LOOKBACK_4H)
    df1d_raw = fetch_live_oanda_candles(symbol, "D", LOOKBACK_1D)
    
    if df5_raw.empty or df1_raw.empty:
        log(f"Data missing for {symbol}, skipping...")
        return
    ...
    csv_path = BASE_LOG_DIR / f"Strategy17_M3M2_Var1_{symbol}_SELL_5min.csv"
    ...
    for edt, idxs in entry_map.items():
        if len(idxs) < 2: continue
        idxs_sorted = sorted(idxs, key=lambda i: str(rows[i].get(f"{tc} MACD cycle Startime") or ""))
        newest = idxs_sorted[-1]
        for old_i in idxs_sorted[:-1]:
            old = rows[old_i]; new = rows[newest]
            old["Status"] = f"Invalidated due to Sell occured at same Datetime"
            extra = OrderedDict({
                "Invalidation Reason": old["Status"],
                f"{tc} MACD CYCLE start time due to which setup invalidated": new.get(f"{tc} MACD cycle Startime"),
                f"{tc} MACD CYCLE endtime time due to which setup invalidated": new.get(f"{tc} MACD cycle Endtime"),
                "Key candle datetime due to which setup invalidated": new.get("Final Key Candle Datetime"),
                f"Final sell datetime": edt,
            })
            old_add = old.get("Strategy Additional info") or ""
            old["Strategy Additional info"] = old_add + "\n\n" + vkv(extra)
            for prefix in ["2hrs", "4hrs", "1D"]:
            ...
            for r_ in RATIOS_FULL:
                ...
                if r_ >= 2: old[f"Status 1:{r_}"] = None
            for _, col in VOL_COL_MAP.items(): old[col] = None
            old["Final Sell found from which Method"] = None

    # Processing and Telegram logic
    for row in rows:
        fcc_ts = row.get("Final Strategy Candle Datetime")
        row["Logged At UTC"] = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
        
        # Give it a unique ID based on FCC TS and symbol
        ev_id = hashlib.sha256(f"sell|{symbol}|{fcc_ts}".encode()).hexdigest()[:24]
        row["Event ID"] = ev_id
        ...
        if row.get("Status") != "Intrade" and "Invalidated" in row.get("Status", ""):
            ...
            if is_new_entry and row.get("Status") == "Intrade":
                ...
                if msg_ev not in _fired_events:
                    _fired_events.add(msg_ev)
                    msg = format_telegram(row, symbol)
                    log(f"\n🔔 NEW SIGNAL [{symbol}]:\n{msg}")
                    tg_post(msg)
...
def main():
    log("=" * 60)
    log("🚀 Strategy 17 M3 M2 Variation 1 FOREX LIVE SCANNER")
    log("=" * 60)
    
    load_fired_events()
    syms_str = ", ".join(INSTRUMENTS)
    tg_post(f"🚀 Strategy 17 M3 M2 Variation 1 LIVE — STARTED\nSymbols: {syms_str}\nMode: SELL side only")
    ...
    try:
        while True:
            ...
            if now.second <= 15 and scan_key != last_scan_key:
                log(f"\n[{now.strftime('%Y-%m-%d %H:%M:%S UTC')}] Scanning...")
                for inst in INSTRUMENTS:
                    try:
                        run_live_scan_for_instrument(inst)
                    except Exception as e:
                        log(f"Error scanning {inst}: {e}")
