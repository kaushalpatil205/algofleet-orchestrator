...
"""
Strategy 18.1 (1H-5min) Live Bridge — BTCUSDT
==============================================
Timeframes: 1H (setup zone + setup finding + entry) → 5min (trailing SL)
...
import trade_db
trade_db.init("S18.1-1H-5min-BTCUSDT-LIVE" + _config.get("STRATEGY_ID_SUFFIX", ""), _config.get("TRADE_DB_URL", ""), magic=MAGIC, bridge_url=MT5_BRIDGE_URL)
...
LOOKBACK_1H = 2000    # usable HTF candles (zone + setup finding)
LOOKBACK_5M = 6000    # usable LTF candles (entry, hard SL, trailing)

WARMUP_1H = 250      # BB/MACD/EMA only -> settles fast
WARMUP_5M = 2000     # KamaLine needs ~757 worst case; 2.6x margin

FETCH_1H = LOOKBACK_1H + WARMUP_1H
...
SCAN_SLEEP_SEC    = 60
RECENT_LTF_COUNT  = 10              # how many recent 5min candles count as "new"
...
def map_htf_to_mtf_start(htf_ts):
    """Next 5min candle after 1H candle closes."""
...
def find_setups_in_zone(arr1d: Dict[str, Any], zone_start_pos: int, zone_end_pos: int, side: str) -> List[Dict[str, Any]]:
    """
    Scans 1H candles within a zone for anchor → trigger → gc candle sequences.
...
def find_entry_multiday(
    ...
    arr1d:       Dict[str, Any],   # 1H data  — for "previous 1H candle" open/close level
    gc_ts:       pd.Timestamp,
    gc_open:     float,
    ...
    """
    Scans 5min candles for entry. Uses 1H candles to get the 'day level' (previous 1H open/close).
    ...
    # Start 5min scanning at the next 5min candle after the gc_ts 1H candle
    ...
    # Period 1: level = gc_open (the 1H candle's open that triggered setup)
    ...
    for pos in range(start_pos_1h, ze_pos_1h + 1):
        ...
        # ---------- Period rollover (based on 1H candle boundary) ----------
...
def compute_hard_sl_1h(arr1h: Dict[str, Any], entry_pos_1h: int, side: str) -> Dict[str, Any]:
    """
    arr1h = 5min data.
    SELL: highest of [Highest High, 5 indicators] across prev 5 5min candles.
    BUY:  lowest of same.
    """
...
def run_backtest_5m(
    ...
    Tracking starts from the 5min candle after entry_ts.
...
def save_fired_events():
    try:
        with open(BASE_LOG_DIR / "_fired_events_1H-5min.json", "w") as f:
...
def run_strategy18_1(arr1d, arr1h, arr5m, side: str, recent_ltf: pd.DatetimeIndex) -> List[OrderedDict]:
    ...
    for zone in reversed(zones):
        ...
        for setup in reversed(setups):
            ...
            # Multi-period entry search on 5min candles
            try:
            ...
            # Hard SL computation from 5 previous 5min candles
...
def run_live_scan():
    ...
    global _fired_events
    global _fired_entry_timestamps
    log("Fetching live candles (1H + 5min only)...")

    from concurrent.futures import ThreadPoolExecutor
    with ThreadPoolExecutor(max_workers=2) as pool:
        f_1h = pool.submit(get_rolling_mt5_candles, SYMBOL, "H1", FETCH_1H)
        f_5m = pool.submit(get_rolling_mt5_candles, SYMBOL, "M5", FETCH_5M)

    df_htf = f_1h.result().iloc[:-1]  # drop last incomplete 1H candle
    df_ltf = f_5m.result().iloc[:-1]  # drop last incomplete 5min candle
    ...
    # arr5m carries raw OHLCV only, but is kept index-identical to arr1h so an
    # entry and its trailing simulation always share the same candle universe.
    df_ltf_used = df_ltf.loc[df_ltf.index.isin(df_ltf_prep.index)]

    log(f"[DATA] 1H: {len(df_htf_prep):,} usable bars "
        f"({df_htf_prep.index[0]} -> {df_htf_prep.index[-1]})")
    log(f"[DATA] 5M: {len(df_ltf_used):,} usable bars "
        f"({df_ltf_used.index[0]} -> {df_ltf_used.index[-1]})")
...
def main():
    log("=" * 70)
    log("🚀 Strategy 18.1 (1H-5min) LIVE BRIDGE — BTCUSDT")
    log("   Timeframes: 1H (setup/entry) → 5min (trailing SL)")
