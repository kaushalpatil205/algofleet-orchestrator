...
"""
Strategy 18.1 (1H-5min) Live Bridge — XAUUSD  (Exness / MT5 bridge)
===================================================================
...
STRATEGY LOGIC SOURCE
  ...
  S18.1_1H-5min.py (the XAUUSD backtest).  Nothing from the BTCUSDT
...
import trade_db
trade_db.init("S18.1-1H-5min-XAUUSD-LIVE" + _config.get("STRATEGY_ID_SUFFIX", ""), _config.get("TRADE_DB_URL", ""),
...
LOOKBACK_1H = 3000   # ≈ 185 calendar days
LOOKBACK_5M = 6000   # ≈ 30 calendar days
...
FETCH_1H = LOOKBACK_1H + WARMUP_1H   # 3500 — the bridge serves this in one call
FETCH_5M = LOOKBACK_5M + WARMUP_5M   # 8000
...
BASE_LOG_DIR   = Path("./bridge/Strategy 18.1 XAUUSD Live Logs")
BASE_LOG_DIR.mkdir(parents=True, exist_ok=True)
_BUY_LOG_PATH  = BASE_LOG_DIR / f"Strategy18.1_1H-5min_{SYMBOL}_BUY_live.csv"
_SELL_LOG_PATH = BASE_LOG_DIR / f"Strategy18.1_1H-5min_{SYMBOL}_SELL_live.csv"
...
def format_telegram(r: dict, side: str, order_id: int = 0) -> str:
    ...
    if side == "buy":
        ...
        bb1_col     = f"1st Candle touching Lower BB on 5min Datetime: {r.get(f'1st Candle touching Lower BB on 5min Datetime','')}"
        ca_col      = f"Final 1H 5min Candle closing above 1H TF level DT: {r.get(f'Final 1H 5min Candle closing above 1H TF level DT','')}"
        cb_col      = f"Final 1H 5min Candle closing above Highest Level DT: {r.get(f'Final 1H 5min Candle closing above Highest Level DT','')}"
        up_bb_col   = f"Final 1H 5min Candle touching Upper BB DT: {r.get(f'Final 1H 5min Candle touching Upper BB DT','')}"
    else:
        ...
        bb1_col     = f"1st Candle touching Upper BB on 5min Datetime: {r.get(f'1st Candle touching Upper BB on 5min Datetime','')}"
        ca_col      = f"Final 1H 5min Candle closing below 1H TF level DT: {r.get(f'Final 1H 5min Candle closing below 1H TF level DT','')}"
        cb_col      = f"Final 1H 5min Candle closing below Lowest Level DT: {r.get(f'Final 1H 5min Candle closing below Lowest Level DT','')}"
        up_bb_col   = f"Final 1H 5min Candle touching Lower BB DT: {r.get(f'Final 1H 5min Candle touching Lower BB DT','')}"

    msg = (
        f"{color} {SYMBOL}\n"
        f"Asset: {SYMBOL} (Theoretical)\n"
        f"Strategy: Strategy 18.1 1H-5min Live\n"
        ...
        f"{gc_col}\n"
        f"{bb1_col}\n"
        f"Entry Found according to which 1H level: {r.get(f'Entry Found according to which 1H level','')}\n"
        f"Final 1H Datetime on 1H TF: {r.get(f'Final 1H Datetime on 1H TF','')}\n"
        f"Final 1H 5min Candle touching BB DT: {r.get(f'Final 1H 5min Candle touching BB DT','')}\n"
        f"Final 1H Datetime on 1H TF level: {r.get(f'Final 1H Datetime on 1H TF level','')}\n"
        f"{up_bb_col}\n"
        f"{ca_col}\n"
        f"{cb_col}\n"
...
def format_telegram_exit(r: dict, side: str, ratio_str: str, msg_type: str,
    ...
    return (
        f"Asset: {SYMBOL} (Theoretical)\n"
        f"Strategy: Strategy 18.1 1H-5min Live (XAUUSD)\n"
        f"Entry Signal: {side}\n"
        f"Trade Entry Id: {r.get('Event ID','')}\n"
...
def load_fired_events():
    ...
    if (BASE_LOG_DIR / "_fired_events_1H-5min_XAUUSD.json").exists():
        try:
            with open((BASE_LOG_DIR / "_fired_events_1H-5min_XAUUSD.json"), "r") as f:
...
def save_fired_events():
    try:
        with open(BASE_LOG_DIR / "_fired_events_1H-5min_XAUUSD.json", "w") as f:
...
def run_strategy18_1(arr1d, arr1h, arr5m, side: str, recent_ltf: pd.DatetimeIndex) -> List[OrderedDict]:
    ...
    for zone in reversed(zones):
        ...
        for setup in reversed(setups):
            ...
            entry_candle_close = entry_ts_pd + pd.Timedelta(minutes=5)
...
def run_live_scan():
    ...
    log("Fetching live candles (1H + 5min)...")

    from concurrent.futures import ThreadPoolExecutor
    with ThreadPoolExecutor(max_workers=2) as pool:
        f_htf = pool.submit(get_rolling_mt5_candles, SYMBOL, TF_HTF, FETCH_1H)
        f_ltf = pool.submit(get_rolling_mt5_candles, SYMBOL, TF_LTF, FETCH_5M)

    df_htf_raw = f_htf.result()
    df_ltf_raw = f_ltf.result()

    if df_htf_raw.empty or df_ltf_raw.empty:
        log("Data missing, skipping scan.")
        return

    # Drop the last incomplete (still-forming) candle from each timeframe
    df_htf = df_htf_raw.iloc[:-1]
    df_ltf = df_ltf_raw.iloc[:-1]   # raw 5min — kept full for arr5m trailing backtest

    # Add TradingView FX session columns to BOTH timeframes
    df_htf = prepare_live_frame(df_htf)
    df_ltf = prepare_live_frame(df_ltf)

    # --- arr1d role: 1H candles with MACD cycles + all indicators ---
    df_htf_prep = add_raw_macd_cycles(prepare_df(df_htf))
    if len(df_htf_prep) > WARMUP_1H:
        df_htf_prep = df_htf_prep.iloc[WARMUP_1H:]

    # --- arr1h role: 5min candles with indicators (entry search + hard SL) ---
    df_ltf_prep = prepare_df(df_ltf)
    if len(df_ltf_prep) > WARMUP_5M:
        df_ltf_prep = df_ltf_prep.iloc[WARMUP_5M:]

    # --- arr5m role: raw 5min OHLCV over the SAME rows as arr1h (trailing SL) ---
    # Kept index-identical to df_ltf_prep on purpose: an entry can only be found
    # on a bar arr1h can see, so trailing never needs rows outside that slice,
    # and sharing one index keeps searchsorted positions consistent between the
    # two arrays.  arr5m simply carries no indicator columns.
    df_ltf_used = df_ltf.loc[df_ltf.index.isin(df_ltf_prep.index)]

    log_data_summary(df_htf_prep, TF_HTF)
    log_data_summary(df_ltf_prep, TF_LTF)

    dump_fetched_data(df_htf_prep, TF_HTF)
    dump_fetched_data(df_ltf_used, TF_LTF)

    # Build fast numpy arrays:
    #   arr1d = 1H indicators + MACD (zone/setup/day-level logic)
    #   arr1h = 5min indicators        (entry candle scan + BB touch + hard SL)
    #   arr5m = 5min OHLCV only        (trailing SL simulation / backtest)
    ...
    if "session_date" not in arr1d:
        log("[WARN] 1H arrays carry no session_date — day rollover would be wrong. Skipping scan.")
        return
...
def main():
    log("=" * 70)
    log("🚀 Strategy 18.1 (1H-5min) LIVE BRIDGE — XAUUSD (Exness)")
    log("   Timeframes: 1H (setup/day levels) → 5min (entry + trailing SL)")
    ...
    log(f"   Window: 1H {LOOKBACK_1H}+{WARMUP_1H} warm-up | "
        f"5min {LOOKBACK_5M}+{WARMUP_5M} warm-up")
    log("=" * 70)
    ...
    tg_post(
        f"🚀 Strategy 18.1 (1H-5min) LIVE — STARTED\n"
        f"Symbol: {SYMBOL}\n"
        f"Mode: BUY + SELL\n"
        f"Timeframes: 1H (setup) → 5min (entry + trailing)\n"
