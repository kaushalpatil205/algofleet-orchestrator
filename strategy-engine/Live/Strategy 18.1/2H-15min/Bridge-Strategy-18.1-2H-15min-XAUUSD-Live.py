...
import trade_db
trade_db.init("S18.1-2H-15min-XAUUSD-LIVE" + _config.get("STRATEGY_ID_SUFFIX", ""), _config.get("TRADE_DB_URL", ""),
...
BASE_LOG_DIR   = Path("./bridge/Strategy 18.1 2H-15min XAUUSD Live Logs")
...
def load_fired_events():
    ...
    if (BASE_LOG_DIR / "_fired_events_2H-15min_XAUUSD.json").exists():
        try:
            with open((BASE_LOG_DIR / "_fired_events_2H-15min_XAUUSD.json"), "r") as f:
...
def main():
    log("=" * 70)
    log("🚀 Strategy 18.1 (2H-15min) LIVE BRIDGE — XAUUSD (Exness)")
    log("   Timeframes: 2H (setup/day levels) → 15min (entry) → 5min (trailing SL)")
    ...
    tg_post(
        ...
        f"Symbol: {SYMBOL}\n"
        f"Mode: BUY + SELL\n"
        f"Timeframes: 2H (setup) → 15min (entry) → 5min (trailing)\n"
