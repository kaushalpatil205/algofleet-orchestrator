...
"""
Strategy 18.1 (2H-15min) Live Bridge — BTCUSDT
==============================================
Timeframes: 2H (setup) → 15min (entry) → 5min (trailing SL)

NOTE ON INTERNAL VARIABLE NAMES:
  arr1d  → holds 2H candle data  (zone/setup finding, day-level lookups)
...
import trade_db
trade_db.init("S18.1-2H-15min-BTCUSDT-LIVE" + _config.get("STRATEGY_ID_SUFFIX", ""), _config.get("TRADE_DB_URL", ""), magic=MAGIC, bridge_url=MT5_BRIDGE_URL)
...
LOOKBACK_2H = 2000    # usable HTF candles (zone + setup finding)
LOOKBACK_15M = 6000    # usable LTF candles (entry, hard SL, trailing)

WARMUP_2H = 250      # BB/MACD/EMA only -> settles fast
WARMUP_15M = 2000     # KamaLine needs ~757 worst case; 2.6x margin
WARMUP_5M = 2000

FETCH_2H = LOOKBACK_2H + WARMUP_2H
...
_BUY_LOG_PATH  = BASE_LOG_DIR / f"Strategy18.1_2H-15min_{SYMBOL}_BUY_live.csv"
_SELL_LOG_PATH = BASE_LOG_DIR / f"Strategy18.1_2H-15min_{SYMBOL}_SELL_live.csv"
...
def map_htf_to_mtf_start(htf_ts):
    """Next 15min candle after 2H candle closes."""
...
def find_setups_in_zone(arr1d: Dict[str, Any], zone_start_pos: int, zone_end_pos: int, side: str) -> List[Dict[str, Any]]:
    """
    Scans 2H candles within a zone for anchor → trigger → gc candle sequences.
...
def find_entry_multiday(
    ...
    arr1d:       Dict[str, Any],   # 1H data  — for "previous 2H candle" open/close level
    gc_ts:       pd.Timestamp,
    gc_open:     float,
    ...
    """
    Scans 15min candles for entry. Uses 2H candles to get the 'day level' (previous 1H open/close).
    ...
    # Start 5min scanning at the next 15min candle after the gc_ts 2H candle
    ...
    # Period 1: level = gc_open (the 2H candle's open that triggered setup)
    ...
    for pos in range(start_pos_1h, ze_pos_1h + 1):
        ...
        # ---------- Period rollover (based on 2H candle boundary) ----------
...
def format_telegram(r: dict, side: str, order_id: int = 0) -> str:
    ...
    if side == "buy":
        ...
        ca_col      = f"Final 2H 15min Candle closing above 2H TF level DT: {r.get(f'Final 2H 15min Candle closing above 2H TF level DT','')}"
        cb_col      = f"Final 2H 15min Candle closing above Highest Level DT: {r.get(f'Final 2H 15min Candle closing above Highest Level DT','')}"
        up_bb_col   = f"Final 2H 15min Candle touching Upper BB DT: {r.get(f'Final 2H 15min Candle touching Upper BB DT','')}"
    else:
        ...
        ca_col      = f"Final 2H 15min Candle closing below 2H TF level DT: {r.get(f'Final 2H 15min Candle closing below 2H TF level DT','')}"
        cb_col      = f"Final 2H 15min Candle closing below Lowest Level DT: {r.get(f'Final 2H 15min Candle closing below Lowest Level DT','')}"
        up_bb_col   = f"Final 2H 15min Candle touching Lower BB DT: {r.get(f'Final 2H 15min Candle touching Lower BB DT','')}"

    msg = (
        f"{color} {SYMBOL}\n"
        f"Asset: {SYMBOL} (Theoretical)\n"
        f"Strategy: Strategy 18.1 2H-15min Live\n"
        ...
        f"{gc_col}\n"
        f"{bb1_col}\n"
        f"Entry Found according to which 2H level: {r.get(f'Entry Found according to which 2H level','')}\n"
        f"Final 2H Datetime on 2H TF: {r.get(f'Final 2H Datetime on 2H TF','')}\n"
        f"Final 2H 15min Candle touching BB DT: {r.get(f'Final 2H 15min Candle touching BB DT','')}\n"
        f"Final 2H Datetime on 2H TF level: {r.get(f'Final 2H Datetime on 2H TF level','')}\n"
        f"{up_bb_col}\n"
        f"{ca_col}\n"
        f"{cb_col}\n"
...
def save_fired_events():
    try:
        with open(BASE_LOG_DIR / "_fired_events_2H-15min.json", "w") as f:
...
def run_live_scan():
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        f_1h = pool.submit(get_rolling_mt5_candles, SYMBOL, "H2", FETCH_2H)
    ...
    if len(df_htf_prep) > WARMUP_2H:
        df_htf_prep = df_htf_prep.iloc[WARMUP_2H:]
    ...
    log(f"[DATA] 2H: {len(df_htf_prep):,} usable bars "
...
def main():
    log("=" * 70)
    log("🚀 Strategy 18.1 (2H-15min) LIVE BRIDGE — BTCUSDT")
    log("   Timeframes: 2H (setup) → 15min (entry) → 5min (trailing SL)")
    ...
    tg_post(
        f"🚀 Strategy 18.1 (2H-15min) LIVE — STARTED\n"
        f"Symbol: {SYMBOL}\n"
        f"Mode: BUY + SELL\n"
        f"Timeframes: 2H (setup/entry) → 15min (trailing)\n"
