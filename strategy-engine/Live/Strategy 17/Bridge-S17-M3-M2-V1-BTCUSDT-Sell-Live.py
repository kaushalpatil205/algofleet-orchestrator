...
# -*- coding: utf-8 -*-
"""
Strategy 17 — Method 3 & Method 2 — Variation 1
-------------------------------------------------
...
import trade_db
trade_db.init("S17-M3M2-V1-BTCUSDT-SELL", _config.get("TRADE_DB_URL", ""), magic=MAGIC, bridge_url=MT5_BRIDGE_URL)


#TELEGRAM_BOT_TOKEN=8799287788:AAHS8MiKk5cNjJrfXYFzEaDrmYqpOTXSBuo
#TELEGRAM_CHAT_ID=-1003973537841
...
BASE_LOG_DIR = Path("./bridge/Strategy 17 M3 M2 Variation 1 Live Logs")
BASE_LOG_DIR.mkdir(parents=True, exist_ok=True)
_SELL_LOG_PATH = BASE_LOG_DIR / f"Strategy17_M3M2_Var1_{COIN_NAME}_SELL_5min.csv"
...
def run_strategy17_variation1(df5, df1, volcalc1, arr2h, arr4h, arr1d, side):
    ...
    for edt, idxs in entry_map.items():
        ...
        for old_i in idxs_sorted[:-1]:
            ...
            # Clear trade fields
            for fld in ["Entry Datetime", "Entry Price", "Hard SL Price",
...
#   "entry_price": float,
#   "hard_sl": float,
#   "targets": {ratio: price, ...},   # 1:0.5, 1:1, 1:2, ..., 1:10
#   "current_sl": float,              # last ratcheted SL
#   "trail_hit": set of ratios whose target has been crossed
# }
# Populated whenever a new trade is opened via mt5_bridge_trade.
_ticket_map: Dict[int, Dict[str, Any]] = {}
_event_to_ticket: dict = {}

def load_fired_events():
    global _fired_events
    # Load persisted fired events from JSON to survive restarts
    try:
        import json
        ...
        log(f"[load_fired_events] json error: {_e}")
    if _SELL_LOG_PATH.exists():
        try:
            df = pd.read_csv(_SELL_LOG_PATH, dtype=str)
...
def log_trade_error(symbol: str, error_msg: str):
    try:
...
def mt5_bridge_trade(symbol: str, action_type: int, volume: float, sl: float = 0.0):
    ...
    try:
        ...
        payload = {
            "action": 1,        # 1 = DEAL (Market)
            "symbol": symbol,
            "volume": v,
            "type": action_type, # 0 = Buy, 1 = Sell
            "price": 0.0,
            "sl": s,
            "magic": MAGIC,
            "comment": "S17 Bridge"
...
def mt5_bridge_modify_sl(ticket: int, new_sl: float):
    """Ratchet the SL on an open position.  Used by the trailing-stop
    logic in run_strategy17_variation1's apply_trail path.
    Returns True when the bridge confirms the modify."""
    try:
...
# ================================================================
# TRAILING STOP  (ratio-based, mirrors run_backtest_v1's apply_trail)
# ================================================================
CONSERVATIVE_TRAIL_INTERVAL_SEC = 5   # check positions every 5s
_last_trail_ts = [0.0]                  # mutable so the poll can update it
...
def trail_conservative_positions(positions: list, current_px: float, inst_symbol: str,
    ...
    """For each open conservative (magic=MAGIC) position, walk through
    the ratio targets and ratchet the SL forward whenever a higher
    ratio target has been crossed.  Mirrors the apply_trail logic in
    run_backtest_v1.
    """
    if not positions:
        return
    ...
    for p in positions:
        try:
            ...
            continue
        if magic != MAGIC or ticket <= 0:
            continue
        if ticket not in _ticket_map:
            continue  # we don't have the targets for this ticket — skip
        ...
        # Walk ratios from 2 upward, for as long as price keeps reaching them
        ...
        _hit_px = (low_px if low_px is not None else current_px) if side == 1 else (high_px if high_px is not None else current_px)
        for cp in _trail_checkpoints(rec, _hit_px):
            if cp in rec["trail_hit"]:
                continue
            tgt = _ratio_target(rec, cp)
            if tgt is None:
                continue
            hit = (_hit_px <= tgt) if side == 1 else (_hit_px >= tgt)
            if not hit:
                continue
            rec["trail_hit"].add(cp)
            # Anchor = targets[cp - 2] for cp >= 4, or entry for cp == 2
            if cp == 2:
            ...
            if anchor is None:
                continue
            # SELL: SL is above entry, trails DOWN → skip if anchor >= cur_sl (not an improvement)
            # BUY:  SL is below entry, trails UP   → skip if anchor <= cur_sl (not an improvement)
            if side == 1:  # SELL
                if cur_sl > 0 and anchor >= cur_sl:  # anchor not lower → no improvement
                    continue
            else:  # BUY
                if cur_sl > 0 and anchor <= cur_sl:  # anchor not higher → no improvement
                    continue
            new_sl = round(float(anchor), 6)
            log(f"[TRAIL] ticket={ticket} cp=1:{cp} anchor={new_sl} "
                f"(target hit={tgt}, cur_px={current_px})")
...
def format_telegram(r: dict) -> str:
    ...
    return (
        f"🔴 SELL {SYMBOL}\n"
        f"Strategy: Strategy 17 M3 M2 Variation 1 LIVE\n"
        f"Final Strategy Candle Datetime: {r.get('Final Strategy Candle Datetime','')}\n"
        f"Final Sell found from which Method: {r.get('Final Sell found from which Method','')}\n"
...
def run_live_scan():
    ...
    for s5 in setups5:
        ...
        ev_id = hashlib.sha256(f"sell|{fcc_ts}".encode()).hexdigest()[:24]
        row["Event ID"] = ev_id
        
        # Check if entry is new and finalized
        fin_ts = pd.Timestamp(row.get("Entry Datetime")) if row.get("Entry Datetime") else None
        ...
        if row.get("Status") != "Intrade" and "Invalidated" in row.get("Status", ""):
            ...
            row.pop("_side", None)
            rows_to_save.append(row)
            # In V1, Entry Datetime is the trigger (no target wait)
            if is_new_entry and row.get("Status") == "Intrade":
                ...
                if msg_ev not in _fired_events:
                    ...
                    log(f"\n🔔 NEW SIGNAL:\n{msg}")
                    trade_db.record_signal(ev_id, SYMBOL_BRIDGE, "sell",
                                           signal_price=row.get("Entry Price"),
                                           qty=row.get("Qty"),
                                           hard_sl=row.get("Hard SL Price"),
                                           extra={"method": row.get("Final Sell found from which Method")})

                    # --- Send to MT5 Bridge ---
                    qty     = row.get("Qty", 0.01)
                    hard_sl = row.get("Hard SL Price", 0.0)
                    # action_type=1 for SELL; round qty to 0.01 lot step
                    # so the broker doesn't reject with Invalid volume.
                    try:
                        raw_qty = float(qty) if qty not in (None, "", "None") else 0.01
                    except (ValueError, TypeError):
                        raw_qty = 0.01
                    raw_qty = max(raw_qty, 0.01)
                    rounded_qty = round(raw_qty / 0.01) * 0.01
                    rounded_qty = round(rounded_qty, 2)
                    ...
                    if t_qty not in (None, "", "None") and not pd.isna(t_qty):
                        try:
                            ...
                            pass
                    order_id, retcode, t_comment = mt5_bridge_trade(SYMBOL_BRIDGE, 1, rounded_qty, hard_sl)
                    ...
                    # Add execution status to message and post
                    msg += exec_status
                    tg_post(msg)
                    trade_db.mark_telegram_sent(ev_id)

                    if order_id > 0 and retcode == 10009:
                        # Build the targets dict from the row's 1:N columns so
                        # the trailing logic knows where to ratchet SL to.
                        targets = {}
                        base_entry = actual_entry if actual_entry is not None else float(row.get("Entry Price", 0) or 0)
                        safe_hard_sl = float(hard_sl or 0)
                        for r_ in RATIOS_FULL:
                            targets[r_] = _tgt(base_entry, safe_hard_sl, "sell", r_)
                        targets[0.5] = _tgt(base_entry, safe_hard_sl, "sell", 0.5)
                        ...
                        trade_db.record_execution(ev_id, order_id, retcode,
                                                  ...
                                                  targets=targets)
                        log(f"[TRAIL] registered ticket={order_id} entry={_ticket_map[order_id]['entry_price']} "
                            f"hard_sl={hard_sl} targets={ {k: round(v,2) for k,v in targets.items()} }")
                        # Telegram: position opened (with same ticket MT5 returned)
                        tg_post(f"✅ POSITION OPENED\n"
                                f"Ticket: {order_id}\n"
                                f"Entry: {_ticket_map[order_id]['entry_price']}\n"
                                f"Hard SL: {hard_sl}\n"
                                f"Qty: {rounded_qty}\n"
                                f"Strategy: S17 M3 M2 V1")
                    else:
                        trade_db.record_execution(ev_id, order_id, retcode, error=t_comment)
                        # Telegram: MT5 rejected/failed
                        tg_post(f"⚠️ MT5 ORDER FAILED\n"
                                f"{msg}\n"
                                f"(no ticket returned from bridge)")

            # Only send Telegram alerts for rows with a live MT5 ticket
            if row.get("Event ID") in _event_to_ticket:
                process_telegram_exits(row, SYMBOL, "sell", "Strategy 17 M3 M2 Variation 1", recent_1m)
    ...
    if rows_to_save:
        ...
        if _SELL_LOG_PATH.exists():
            try:
                old_df = pd.read_csv(_SELL_LOG_PATH)
        ...
        merged.to_csv(_SELL_LOG_PATH, index=False)
...
def main():
    log("=" * 60)
    log("🚀 Strategy 17 M3 M2 Variation 1 LIVE SCANNER")
    log("=" * 60)

    load_fired_events()
    recover_open_trades()
    tg_post("🚀 Strategy 17 M3 M2 Variation 1 LIVE — STARTED\nSymbol: BTCUSDT\nMode: SELL side only\nNew code with FIXED_RISK_USD and slippage adjustment.\nFixed SL Trailing Logic")
