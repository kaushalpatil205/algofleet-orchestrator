...
import trade_db
trade_db.init("S17-M2M3-V4-FOREX-BUY", _config.get("TRADE_DB_URL", ""), magic=MAGIC, bridge_url=MT5_BRIDGE_URL)
...
BASE_LOG_DIR = Path("./bridge/Strategy 17 M3 M2 Variation 4 Forex Live Logs")
        ...
        if row.get("Status") != "Intrade" and "Invalidated" in row.get("Status", ""):
            ...
            if is_new_entry and row.get("Status") == "Intrade":
                ...
                if msg_ev not in _fired_events:
                    ...
                    log(f"\n🔔 NEW SIGNAL [{symbol}]:\n{msg}")
                    trade_db.record_signal(ev_id, symbol, "buy",
