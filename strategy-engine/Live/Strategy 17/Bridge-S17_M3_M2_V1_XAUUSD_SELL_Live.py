...
import trade_db
trade_db.init("S17-M3M2-V1-XAUUSD-SELL", _config.get("TRADE_DB_URL", ""), magic=MAGIC, bridge_url=MT5_BRIDGE_URL)
...
BASE_LOG_DIR = Path("./bridge/Strategy 17 M3 M2 Variation 1 Forex Live Logs")
...
def process_telegram_exits(row: dict, symbol: str, side: str, strategy_name: str, recent_1m: pd.DatetimeIndex):
    ...
    for key, r_list in sl_groups.items():
        ...
        if msg_ev not in _fired_events and exit_ts in recent_1m:
            ...
            _fired_events.add(msg_ev)
            if len(r_list) == 1:
                ratio_str = f"1:{r_list[0]}"
            else:
                ratio_str = f"1:{r_list[0]} to 1:{r_list[-1]}"
        ...
        if row.get("Status") != "Intrade" and "Invalidated" in row.get("Status", ""):
            ...
            if is_new_entry and row.get("Status") == "Intrade":
                ...
                if msg_ev not in _fired_events:
                    ...
                    log(f"\n🔔 NEW SIGNAL [{symbol}]:\n{msg}")
                    trade_db.record_signal(ev_id, symbol, "sell",
                    ...
                    order_id, retcode, t_comment = mt5_bridge_trade(symbol, 1, rounded_qty, hard_sl)
